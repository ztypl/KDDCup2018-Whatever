...
f=open('/Users/xuedi/course/ml/KDD/datasets/beijing_17_18_aq.csv')
f2=open('/Users/xuedi/course/ml/KDD/cleared_datasets/beijing_17_18_aq.csv','w')
cnt=0
dict={}
schema=f.readline()
...
def getTimeApproximate(zone,i,j):
    ...
    if i>=1 and dict[zone][i-1][j]:
        ...
        cnt+=1
    if i+1<8886 and dict[zone][i+1][j]:
        sum+=float(dict[zone][i+1][j])
        cnt+=1
    if i+2<8886 and dict[zone][i+2][j]:
        sum+=float(dict[zone][i+2][j])
        cnt+=1
    if i+3<8886 and dict[zone][i+3][j]:
        sum+=float(dict[zone][i+3][j])
        cnt+=1
    if cnt==0:
        return 0
